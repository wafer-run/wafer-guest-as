...
export async function asp(argv) {
    ...
    const aspectConfig = (await import("file://" + configLocation)).default;
    stdout.write(`Using config: ${configLocation}\n`);
    stdout.write(`ASC Version: ${ascVersion}\n`);
    // filter entries using array of regexp
    let entryFilterRegexes = [];
    if (opts.disclue)
        entryFilterRegexes.push(new RegExp(opts.disclude));
    if (aspectConfig.disclude)
        entryFilterRegexes.push(...aspectConfig.disclude);
    const filterEntry = (str) => entryFilterRegexes.reduce((left, right) => left && !right.test(str), true);
    ...
    if (opts.include)
        includedGlobs.push(...opts.I.split(","));
    if (aspectConfig.include)
        includedGlobs.push(...aspectConfig.include);
    // if no files are specified to be included, then we include the following glob pattern
    if (includedGlobs.length === 0)
        includedGlobs.push("assembly/__tests__/**/*.include.ts");
    ...
    for (const entry of entries) {
        ...
        if (opts.showStats)
            process.stdout.write(compiled.stats.toString());
        ...
        const snapshotMode = opts.updateSnapshots
            ? 0 /* SnapshotMode.WriteSnapshots */
            : 1 /* SnapshotMode.CompareSnapshots */;
        const snapshots = snapshotMode === 1 /* SnapshotMode.CompareSnapshots */ && existsSync(snapshotPath)
        ...
        const module = await aspectConfig.instantiate(memory, (...args) => (covers ? covers.installImports(ctx.createImports(...args)) : ctx.createImports(...args)), instantiate, binary);
        ...
        // snapshot mode!
        if (snapshotMode === 1 /* SnapshotMode.CompareSnapshots */) {
            ...
            for (const [diffName, diffResult] of diff.results) {
                if (diffResult.type === 1 /* SnapshotDiffResultType.Added */) {
                    addedSnapshots += 1;
                ...
                if (diffResult.type === 2 /* SnapshotDiffResultType.Removed */) {
                    overallStats.removedSnapshots += 1;
                }
                if (diffResult.type === 0 /* SnapshotDiffResultType.NoChange */) {
                    overallStats.passedSnapshots += 1;
                }
            }
