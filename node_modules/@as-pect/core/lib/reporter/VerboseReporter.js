...
export class VerboseReporter {
    ...
    onGroupStart(group) {
        ...
        /* istanbul ignore next */
        if (group.name)
            this.stdout.write(`[Describe]: ${group.name}\n\n`);
    }
    /**
        ...
        if (test.pass) {
            ...
            if (!test.negated) {
                ...
                if (test.expected) {
                    ...
                    this.stdout.write(`[Expected]: ${expected.negated ? "Not " : ""}${expected.stringify(stringifyIndent2).trimLeft()}\n`);
                }
            }
            /* istanbul ignore next */
            if (test.message) {
    ...
    onFinish(suite) {
        ...
        for (const [name, result] of diff.entries()) {
            ...
            totalCount += 1;
            addedCount += result.type === 1 /* SnapshotDiffResultType.Added */ ? 1 : 0;
            removedCount += result.type === 2 /* SnapshotDiffResultType.Removed */ ? 1 : 0;
            differentCount += result.type === 3 /* SnapshotDiffResultType.Different */ ? 1 : 0;
        }
...
${"~".repeat(80)}\n\n`);
    ...
    onLog(logValue) {
        ...
        const output = logValue.stringify(indent12).trimLeft();
